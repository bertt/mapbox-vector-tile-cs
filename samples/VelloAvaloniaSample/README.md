...
3. Uses VelloSharp's `SparseRenderContext` to render geometries to a bitmap
4. The sparse renderer provides CPU-based rendering, eliminating the need for GPU acceleration
5. Renders to an Avalonia `WriteableBitmap` which is then displayed in the UI

This sample demonstrates cross-platform vector tile rendering using the Vello rendering engine integrated with Avalonia UI.
